## Software Tools Collection

This repository contains a collection of software tools specifically designed to automate various build and test tasks on Windows clients. Whether you are a developer working on your local machine or using GitHub Actions for continuous integration, these tools will simplify your workflow and enhance your productivity.

---

### Getting Started
- [Installation](installation.md)
- [Usage Guide](usage.md)
- [Project Setup](setup.md)
- [Architecture Overview](devops-tools-suite-architecture.md)
- [Build Types](buildtypes.md)
- [Versioning](versioning.md)

### Core Tools
- [SDO (sdo.exe)](sdo-net.md) - Unified CLI for GitHub, Azure DevOps, and local automation
- [GitHub Releases](github-release.md)

### Development & Modules
- [PowerShell Module - ntools-scripts](ntools-scripts-module.md)
- [ntools-scripts Module API](ntools-scripts-module-api.md)
- [Development Tools](ntools.md)
- [Custom Build Tasks (Nbuild Tasks)](nbuildtasks.md)

### Build System Features
- [Code Coverage](code-coverage.md)
- [SDO Targets](sdo-targets.md)

### DevOps Integration
- [Version Automation Guide](version-automation-guide.md)
- [AzDO Payloads Reference](azdo_payloads.md)

### Work Item Examples
- [AzDO Bug Example](https://github.com/naz-hage/.github/blob/main/.github/skills/templates/issue-azdo-bug-example.md)
- [AzDO Epic Example](https://github.com/naz-hage/.github/blob/main/.github/skills/templates/issue-azdo-epic-example.md)
- [AzDO PBI Example](https://github.com/naz-hage/.github/blob/main/.github/skills/templates/issue-azdo-pbi-example.md)
- [AzDO Task Example](https://github.com/naz-hage/.github/blob/main/.github/skills/templates/issue-azdo-task-example.md)
- [GitHub Issue Example](https://github.com/naz-hage/.github/blob/main/.github/skills/templates/issue-gh-example.md)

---

Don't hesitate to write an [issue](https://github.com/naz-hage/NTools/issues) if you have any questions or suggestions.