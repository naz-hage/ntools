    ...
    try:
        ...
        try:
            ...
            if hasattr(platform, "client") or hasattr(platform, "_ensure_client"):
                ...
                if hasattr(platform, "client") and platform.client:
                    ...
                    if not work_item:
                        raise ValidationError(
                            f"Work item {work_item_id} does not exist or is not accessible."
                        )
        ...
        if getattr(args, "verbose", False):
            ...
            if "work item" in error_str and "not exist" in error_str:
                ...
                print(
                    "[ERROR] Failed to create PR: An active pull request already exists for this branch"
                )
            else:
...
def cmd_pr_status(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print(f"Author: {pr_details.get('author', 'N/A')}")
        print(
            f"Branch: {pr_details.get('source_branch', 'N/A')} → {pr_details.get('target_branch', 'N/A')}"
        )
                        ...
                        if response.status_code == 200:
                            ...
                            if statuses:
                                ...
                                for status in statuses:
                                    ...
                                    print(
                                        f"{state_icon}  {context_name:<40} {state_display:<12} {target_url}"
                                    )
                except Exception as e:
...
def cmd_pr_list(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        prs = platform.list_pull_requests(state=platform_state, author=author, limit=limit)
...
def cmd_pr_update(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        success = platform.update_pull_request(
            pr_number=pr_id, title=title, description=description, status=status
        )
