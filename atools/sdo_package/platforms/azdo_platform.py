    ...
    def create_work_item(
        ...
        if dry_run:
            ...
            if acceptance_criteria:
                ...
                print(
                    f"    {len(acceptance_criteria)} items (will be added via dedicated field or appended to description)"
                )
                if self.verbose:
            ...
            if self.verbose:
                ...
                print(
                    f'  Area Path: {metadata.get("area", f"{metadata.get("project", "Project")}\\Area")}'
                )
                print(
                    f'  Iteration Path: {metadata.get("iteration", f"{metadata.get("project", "Project")}\\Iteration")}'
                )
                print(
                    f"  Acceptance Criteria Count: {len(acceptance_criteria) if acceptance_criteria else 0}"
                )
        ...
        if not organization or not project:
            try:
                ...
                print(
                    "   Either configure Git remote for Azure DevOps, or specify 'organization' and 'project' in metadata"
                )
                return None
        ...
        if acceptance_criteria:
            ...
            try:
                ...
                response = requests.get(
                    fields_url, headers={"Authorization": f"Basic {auth_token}"}, timeout=30
                )
                if response.status_code == 200:
                    ...
                    for field in fields_data.get("value", []):
                        ...
                        if "acceptance" in field_name:
                            ...
                            if self.verbose:
                                print(
                                    f"✅ Found acceptance criteria field: {field.get('name')} -> {ac_field_ref}"
                                )
                            break
        ...
        if assignee:
            operations.append({"op": "add", "path": "/fields/System.AssignedTo", "value": assignee})
        ...
        if tags:
            if isinstance(tags, list):
                ...
                tags_str = str(tags)
            operations.append({"op": "add", "path": "/fields/System.Tags", "value": tags_str})
        ...
        if self.verbose:
            ...
            print("   Equivalent az CLI command:")
            az_cmd = f'az boards work-item create --title "{title}" --type "{work_item_type}"'
            az_cmd += (
                f' --project "{project}" --organization "https://dev.azure.com/{organization}"'
            )
            if area_path:
            ...
            print(f"   {az_cmd}")
            print("   (Note: Requires AZURE_DEVOPS_PAT environment variable or 'az login')")
