...
- **Enhanced Install Command**: Added `--name` and `--version` parameters to `sdoo install` for installing applications by name from current directory JSON files
...
### Changed
- **Build System**: Updated `sdo.exe` to use .NET SDK instead of direct MSBuild dependency
