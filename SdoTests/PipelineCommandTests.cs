...
public class PipelineCommandTests
{
    ...
    [Fact]
    public void Constructor_Has10Subcommands()
    {
        var command = new PipelineCommand(_verboseOption);
        Assert.Equal(10, command.Subcommands.Count);
    }
    ...
    public void Constructor_RegistersSubcommandsInAlphabeticalOrder()
    {
        ...
        var expected = new[] { "create", "delete", "lastbuild", "list", "logs", "release", "run", "show", "status", "update" };
        Assert.Equal(expected, subcommandNames);
    }


    [Fact]
    public void Constructor_AddsReleaseSubcommands()
    {
        var command = new PipelineCommand(_verboseOption);
        var releaseCommand = Assert.Single(command.Subcommands, s => s.Name == "release");


        Assert.Equal(new[] { "list", "show" }, releaseCommand.Subcommands.Select(s => s.Name));
    }
